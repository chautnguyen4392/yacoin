...
UniValue getdifficulty(const JSONRPCRequest& request)
{
    ...
    uint256 nTarget = CBigNum().SetCompact(pindex->nBits).getuint256();
...
UniValue getblockheader(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"chaintrust\" : \"0000...1f3\"     (string) Expected number of hashes required to produce the current chain (in hex)\n"
...
UniValue getblock(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            ...
            "  \"bits\" : \"1d00ffff\", (string) The bits\n"
            "  \"difficulty\" : x.xxx,  (numeric) The difficulty\n"
            "  \"chaintrust\" : \"xxxx\",  (string) Expected number of hashes required to produce the chain up to this block (in hex)\n"
            "  \"previousblockhash\" : \"hash\",  (string) The hash of the previous block\n"
            "  \"nextblockhash\" : \"hash\"       (string) The hash of the next block\n"
            "}\n"
...
UniValue gettimechaininfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "gettimechaininfo\n"
            ...
            "  \"bnChainTrust\": \"xxxx\"     (string) total amount of work in active chain, in hexadecimal\n"
            "}\n"
            "\nExamples:\n"
            + HelpExampleCli("gettimechaininfo", "")
            + HelpExampleRpc("gettimechaininfo", "")
        );
...
UniValue getblockbynumber(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2)
        throw std::runtime_error(
            "getblockbynumber <number> [verbose]\n"
            "\nverbose optional to print more detailed tx info\n"
            "\nReturns details of a block with given block-number.\n"
            "\nExamples:\n"
            + HelpExampleCli("getblockbynumber", "")
            + HelpExampleRpc("getblockbynumber", "")
        );
